"""astrbot_plugin_maisoul v6.26.2 —— 麦麦(MaiBot)发言流水线深度复刻 + 管家桥
...
@register(
    "astrbot_plugin_maisoul", "meng", "麦麦发言流水线深度复刻+管家桥+多人格", "6.26.2"
)
class MaiSoulPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # 模型联动取值器（v6.26.2）：与 replyer 生成同一条解析链——
    ...
    async def initialize(self):
        ...
        logger.info(
            f"maisoul v6.26.2 已加载：模式={self.config['mode']} bot={self.config['bot_name']} "
    ...
    async def terminate(self):
        ...
        self.monitor.close()
        logger.info("maisoul v6.26.2 已卸载")
