...
@register("astrbot_plugin_maisoul", "meng", "麦麦发言流水线深度复刻+管家桥+多人格", "6.15.0")
class MaiSoulPlugin(Star):
    ...
    async def initialize(self):
        ...
        logger.info(
            f"maisoul v6.15.0 已加载：模式={self.config['mode']} bot={self.config['bot_name']} "
...
.get("id", "") or "") if emb is not None else "",
    ...
    async def maisoul_cmd(self, event: AstrMessageEvent):
        ...
        if arg in ("on", "off"):
            ...
            yield event.plain_result(
                f"maisoul v6.15.0状态：{'运行中' if self.config['enable'] else '已停用'} | "
    ...
    async def terminate(self):
        logger.info("maisoul v6.15.0 已卸载")
