...
def _schedule_planner(
    ...
    if pl.agent_state == "wait":
        if (not is_group) and not trigger.effective_talk_value(
            cfg, str(event.get_platform_name() or ""), gid, is_group=is_group
        ) <= 0:
            logger.info(
    ...
    pl.last_event = (
        event  # wait 续轮/定时循环复用同一真实 event（候选/注入都挂在 event 上）
    )
    ...
    if send_fn is not None:
        ...
        return _planner_cycle(
            P, umo, platform, gid, st, is_group, send_fn=send_fn, event=event, gen=gen
        )
...
async def _planner_cycle(
    ...
    tool_count_total = [
        4
    ]  # 可见内置工具数（reply/wait/send_emoji/tool_search）+ 已发现 deferred
    ...
    try:
        ...
        while True:
            remaining = MESSAGE_DEBOUNCE_SECONDS - (
                time.time() - (st.last_ext_ts or 0.0)
            )
            if remaining <= 0:
                break
        ...
        deps = planner.PlannerDeps(
            _PlannerHostAdapter(P),
            st,
            eff_cfg,
            event,
            platform,
        ...
        system_prompt = planner.build_planner_system(
            eff_cfg,
            prompt.build_attention_block(
                eff_cfg, gid, platform, is_group, include_chat_prompt=False
            ),
        )
        attention_tail_msg = prompt.chat_attention_tail(
            eff_cfg, gid, platform, is_group
        )
        ...
        for round_index in range(planner.MAX_INTERNAL_ROUNDS):
            ...
            if use_jargon:
                ...
                jargon_block = learning.jargon_reference_block(
                    ...
                    jargon_key,
                    jargon_recent,
                    exclude=injected_jargons or None,
                    matched_out=new_terms,
                )
                if jargon_block:
            ...
            tail_msgs = [
                x
                for x in (
                    planner.build_deferred_reminder(
            ...
            if usage is not None:
                planner_prompt_tokens += int(
                    getattr(usage, "input_other", 0) or 0
                ) + int(getattr(usage, "input_cached", 0) or 0)
                planner_completion_tokens += int(getattr(usage, "output", 0) or 0)
            logger.info(
                f"maisoul planner[{gid}]: LLM 返回 tools={list(getattr(resp, 'tools_call_name', None) or [])}"
            )
            ...
            if analysis and pl.last_analysis:
                ...
                if SequenceMatcher(None, analysis, pl.last_analysis).ratio() > 0.9:
                    logger.info(
                        f"maisoul planner[{gid}]: 本轮思考与上轮过相似，替换为反思提示"
                    )
            ...
            logger.debug(
                f"maisoul planner[{gid}]: 正文 {len(_resp_text(resp))} 字 / "
            ...
            for i, name in enumerate(names):
                ...
                _monitor_stage(
                    P,
                    gid,
                    monitor.STAGE_TOOL_PREFIX + str(name),
                    ...
                    tool_records.append(
                        {
                            ...
                            "tool_args": args,
                            "tool_call_source": "planner",
                            "tool_call_source_label": "deferred",
                            "success": "失败" not in str(result),
                            "duration_ms": (time.time() - tool_started) * 1000,
                            "summary": str(result)[:2000],
                        }
        ...
        _monitor_stage(
            P,
            gid,
            monitor.STAGE_PLANNER_INTERRUPTED,
            "收到外部中断信号",
            agent_state=pl.agent_state,
        )
...
async def _planner_execute_reply(P, deps, reason: str, args: dict) -> str:
    ...
    set_quote = bool(args.get("set_quote", True)) and bool(
        eff_cfg.get("enable_reply_quote", True)
    )
    ...
    deps.st.planner_state().eco_injection = eco_block + (
        "\n\n[用户内容附加]\n" + "\n".join(_extra_part_text(x) for x in eco_extras)
        if eco_extras
        else ""
    )
...
async def _planner_send_emoji(P, deps) -> str:
    ...
    try:
        ...
        if search_tool is None or send_tool is None:
            return (
                "表情包工具不可用（需要 astrbot_plugin_stealer 的 "
        ...
        provider = P.context.get_using_provider()
        recent = "\n".join(
            f"{m.get('name')}: {m.get('text')}" for m in list(deps.st.buffer)[-15:]
        )
