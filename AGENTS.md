...
65. **Reply 组件自带 .text 是被引用者的原文——全量文本必须跳过容器段（2026-09-11 修，用户实报"引用消息被解析成发言文字"）**：aiocqhttp 适配器默认 `get_reply=True`，每条引用消息的 Reply 组件携带完整引用内容（`.chain`/`.text`/`.message_str`/`sender_id`——.text 标注 deprecated 但仍填充=被引用消息的 message_str）。坑 61 的鸭子判定"有非空 .text 即文本段"从 v6.18.1 起就会把引用原文拼进本人发言（[Reply, At(被回复者), Plain(正文)] → "被引用的话 @名字 正文"），一直潜伏到盯推理过程页才暴露。修法=`full_plain_text` 先判 `hasattr(seg, "chain")` 整体跳过（有 chain 属性即 Reply 容器；Plain/At 无此属性）——引用关系只经 `_quote_ids` 进 `<message quote="…">` 属性，内容永不进正文。**教训：跨边界的鸭子类型判定要枚举目标类全字段——框架组件里"看起来是文本"的属性可能装着别人的话；新增组件字段（deprecated 也算）会静默击穿旧判定**。
