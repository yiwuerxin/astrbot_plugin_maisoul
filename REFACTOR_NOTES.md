...
- **P-H 心弦联动**（`xinxian_link` 默认关）：好心弦 facade（get_profile）把好感等级/印象/关系渲染进系统提示词，缺插件静默降级。**v6.22.0 审计警示：与生态注入桥双重注入**——心弦自身经 `on_llm_request` 注入好感档案（群聊），`eco_injection` 开着时该注入已在流入；两开关同开=同一份数据注两遍。P-H 无效果增量（唯一差异是私聊覆盖，心弦数据模型以群为维度基本不成立），属"功能重复且可替代"，建议拆除、待 owner 拍板。
