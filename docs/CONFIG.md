...
| task_models | —（任务级模型绑定 maisoul 扩展） | `[]`（条目 {task, provider, model?}，provider 填源名） |
...
| 字段 | 类型/默认 | 作用 |
|---|---|---|
| mode | str `independent` | independent=麦麦流水线接管；native=触发后交原生 agent；`planner`（默认部署推荐，完全对标 maisaka 决策层，见 MAIBOT_FIDELITY.md） |
...
页面加载时自动从状态清理（旧值 nicknames 会合并进 aliases）：
