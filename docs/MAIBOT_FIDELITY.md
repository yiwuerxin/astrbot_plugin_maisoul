...
- **WAIT/RUNNING 状态机**：群聊 wait 不唤醒，@/提及必回主动触发恢复；思考打断 planner_interrupt_max_consecutive_count=0 默不打断（开启后仅 planner LLM 请求在途可打断、连续计数自然完成才清零——对齐上游 PlannerInterruptController 的 idle/limit 语义，v6.27.1）；空闲指数退避（base15×2^n 封顶 300、起点 2、积压 6 绕过、reply 重置）；wait 到期有积压自动续轮并注入完成回执（build_wait_completed_message 原文；无积压不续轮会让异步工具后的循环静默死亡，坑 26）。
